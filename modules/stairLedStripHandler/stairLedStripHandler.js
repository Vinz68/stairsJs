...
//
// Monitors a "slidesDirectory" for new powerpoint slides (JPG files).
...
function init() {
    ...
    try {
        ...
        logger.info( 'init: Read and parse JSON file: ' + config.slidesFile + ' ...');
        slidesObj = JSON.parse(fs.readFileSync(config.slidesFile, 'utf8'));
    } catch (e) {
        logger.error('init: Read and pars : ' + config.slidesFile + ' has failed.');
        logger.error('init: Error: ' + e);
    }
...
function fileAdded(fileName)
{
    ...
    // After this time the JPG files will be moved to a "images" directory and HTML files will be generated (in the "html" directory) for each JPG file.
...
function updateSlidesJson() {
    ...
    try {
        ...
        fs.writeFile(config.slidesFile, JSON.stringify(slidesObj, null, 4), function (err) {
            if (err) {
...
function generateHtmlPage(JPGFileName, index) {
    ...
    fs.readFile(config.htmlDirectory+'/slide_template.html', 'utf8', function (err, data) {
        ...
        // TODO .....  ./images/  not ALWAYS good; we will use infodisplay subdirectory => /infodispaly/images/
        var replaceByFileName = './images/' + JPGFileName;
        var replaceByFileDescription = 'Cerner Info-Slide-' + index;

        logger.info("Generated HTML page will embed slide#:" + index + " JPG:" + replaceByFileName);

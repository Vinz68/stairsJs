...
class StairsLedStrips {
    ...
    constructor(gpioArray, options) {
        ...
        for(var i = 0; i < gpioArray.length; i++){
            ...
            ledStripArray[i] = new Gpio(gpioArray[i], 'out', 'none', {activeLow: true} );         // output signal, to enable one led-strip
        }
   ...
   onUpDirectionSlow(inMsec) {
        if (!this.activated) {
            console.log("StairsLedStrips::onUpDirectionSlow");

            this.activated = true;
            this.direction = 'upstairs';
            this.ledStripArrayIndex = 0;

            console.log('inMsec = '+ inMsec);
